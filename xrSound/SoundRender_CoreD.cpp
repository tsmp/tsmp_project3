...
BOOL CSoundRender_CoreD::EAXTestSupport(BOOL bDeferred)
{
	...
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_NONE))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_ALLPARAMETERS))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_ROOM))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_ROOMHF))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_ROOMROLLOFFFACTOR))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_DECAYTIME))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_DECAYHFRATIO))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_REFLECTIONS))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_REFLECTIONSDELAY))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_REVERB))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_REVERBDELAY))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_ENVIRONMENT))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_ENVIRONMENTSIZE))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_ENVIRONMENTDIFFUSION))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_AIRABSORPTIONHF))
		support = FALSE;
	if (!EAXQuerySupport(&DSPROPSETID_EAX_ListenerProperties, ext | DSPROPERTY_EAXLISTENER_FLAGS))
		support = FALSE;
	return support;
}
...
void CSoundRender_CoreD::set_master_volume(float f)
{
	if (pBuffer)
	{
		...
		clamp(hw_volume, DSBVOLUME_MIN, DSBVOLUME_MAX);
