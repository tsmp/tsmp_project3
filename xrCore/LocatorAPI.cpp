...
bool CLocatorAPI::is_game_path(string_path path)
{
	for (const auto& i : game_pathes)
	{
		if (xr_strcmp(i, path) == 0)
		{
			return true;
		}
	}
...
void CLocatorAPI::_initialize(u32 flags, LPCSTR target_folder, LPCSTR fs_name)
{
	...
	else
	{
		...
		while (!pFSltx->eof())
		{
			...
			if (is_game_path(root))
				game_pathes.push_back(xr_strdup(id));
		}
...
void CLocatorAPI::check_cached_files(string_path& fname, const file &desc, LPCSTR &source_name)
{
	...
	strcpy_s(fname, sizeof(string_path), fname_in_cache);
}
...
template <typename T>
void CLocatorAPI::file_from_cache(T *&R, string_path &fname, const file &desc, LPCSTR &source_name)
{
#ifdef DEBUG
