...
void game_sv_GameState::net_Export_Update(NET_Packet &P, ClientID const &idTo)
{
	...
	// Ётот стейт уйдет игроку, которому принадлежит. ”становим ему флаг локальности, чтобы он ассоциировал себ€ с ним.
	...
	// —нимем флаг локальности, если устанавливали
...
void game_sv_GameState::UpdateClientPing(xrClientData *client)
{
	...
	{
		// после 30 сек бездействи€ клиент отключаетс€, если 25 сек не обновл€етс€ пинг - то он скорее всего вылетел
		// если игрок зайдет повторно, у него сохранитс€ в статистике последнее врем€ обновлени€ пинга, в таком случае
		// не выводим ничего
...
void game_sv_GameState::OnEvent(NET_Packet &tNetPacket, u16 type, u32 time, ClientID const &sender)
{
	...
	case GAME_EVENT_ON_HIT:
	{
		...
		if (!hitterEntity) // added by andy because of Phantom does not have server entity
		{
			...
			// TSMP: јктер, нанесший хит успел удалитьс€, id в пакете не валиден. Ќо у игрока, нанесшего хит есть новый id
