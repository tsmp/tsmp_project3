...
#endif
	//	Msg("---------CExplosive Explode [%d] frame[%d]",cast_game_object()->ID(), Device.CurrentFrameNumber);
	OnBeforeExplosion();
	//играем звук взрыва
...
void CExplosive::UpdateCL()
{
	...
	if (m_fExplodeDuration < 0.f && m_blasted_objects.empty())
	{
		...
		//		Msg("---------CExplosive OnAfterExplosion [%d] frame[%d]",cast_game_object()->ID(), Device.CurrentFrameNumber);
	}
	else
	{
...
void CExplosive::OnBeforeExplosion()
{
	...
	{
		HideExplosive();
		//	Msg("---------CExplosive OnBeforeExplosion setVisible(false) [%d] frame[%d]",cast_game_object()->ID(), Device.CurrentFrameNumber);
	}
}
...
void CExplosive::ExplodeWaveProcess()
{
	...
	BLASTED_OBJECTS_I I = remove_if(m_blasted_objects.begin(), m_blasted_objects.end(), SRemovePred());
...
void CExplosive::net_Relcase(CObject *O)
{
	...
	BLASTED_OBJECTS_I I = find(m_blasted_objects.begin(), m_blasted_objects.end(), smart_cast<CPhysicsShellHolder *>(O));
