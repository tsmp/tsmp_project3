...
struct CLoader
{
	...
	struct CHelper3
	{
		...
		struct has_value_compare
		{
			...
			static object_type_traits::detail::yes select(object_type_traits::detail::other<typename _P::value_compare> *);
			template <typename _P>
	...
	template <typename T1, typename T2>
	IC static void load_data(pair<T1, T2> &data, M &stream, const P &p)
	{
