...
void game_cl_mp::PlaySndMessage(u32 ID)
{
	...
	for (u32 i = 0; i < m_pSndMessagesInPlay.size(); i++)
	{
		...
		if (pSndMsgIP->priority == SndMsg.priority)
		{
			if (Level().timeServer() > pSndMsgIP->LastStarted + pSndMsgIP->pSound._handle()->length_ms())
				continue;

			u32 Delay = pSndMsgIP->LastStarted + pSndMsgIP->pSound._handle()->length_ms() - Level().timeServer();
	...
	SndMsg.LastStarted = Level().timeServer() + MaxDelay;
