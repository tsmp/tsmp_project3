...
BOOL CCar::net_SaveRelevant() { return TRUE; }
...
void CCar::VisualUpdate(float fov)
{
	...
	if (Owner())
	{
		if (!IsMyCar() && !Owner()->getVisible())
		{
			...
			// голова водител€ смотрит пр€мо перед собой
	...
	//Msg("i.x = %f, i.y = %f, i.z = %f", XFORM().i.x, XFORM().i.y, XFORM().i.z); непон€тные дубликаты k. ѕохоже на разные формы xyz - xzy, zxy и т.д.
...
void CCar::NextUpdate()
{
	...
	// ≈сли пропускаем много апдейтов, то выставим больше 1, чтобы интерпол€ци€ быстрее подхватывала апдейты. ≈сли мало, то меньше 1.
	m_LerpMul = 1.f;
...
void CCar::Interpolate()
{
	...
	if (Device.dwTimeGlobal - m_InterpolationStartTime > 20000) // Ќа вс€кий случай. Ќе должно интерполировать на одних и тех же данных больше 20 сек.
	{
...
void CCar::Hit(SHit *pHDS)
{
	...
	if (Owner() && !LevelIsNullptr() && Level().CurrentEntity() && Owner()->ID() == Level().CurrentEntity()->ID())
		HUD().GetUI()->UIMainIngameWnd->CarPanel().SetCarHealth(GetfHealth() /* /100.f */);
}
...
void CCar::OnEvent(NET_Packet &P, u16 type)
{
	...
	//обработка сообщений, нужных дл€ работы с багажником машины
	u16 id;
	switch (type)
	{
...
void CCar::Teleport(Fvector outPos)
{
	...
	Fmatrix F = XFORM();
	Fvector pos = outPos;
	pos.y += 2.5; // TSMP: на вс€кий случай поднимем повыше точку по€влени€
	F.c = pos;
