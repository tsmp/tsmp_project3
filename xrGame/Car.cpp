...
void CCar::VisualUpdate(float fov)
{
	...
	if (Owner())
	{
		if (!IsMyCar() && !Owner()->getVisible())
		{
			...
			// ãîëîâà âîäèòåëÿ ñìîòðèò ïðÿìî ïåðåä ñîáîé
...
void CCar::NextUpdate()
{
	...
	// Åñëè ïðîïóñêàåì ìíîãî àïäåéòîâ, òî âûñòàâèì áîëüøå 1, ÷òîáû èíòåðïîëÿöèÿ áûñòðåå ïîäõâàòûâàëà àïäåéòû. Åñëè ìàëî, òî ìåíüøå 1.
	m_LerpMul = 1.f;
...
void CCar::Interpolate()
{
	...
	if (Device.dwTimeGlobal - m_InterpolationStartTime > 20000) // Íà âñÿêèé ñëó÷àé. Íå äîëæíî èíòåðïîëèðîâàòü íà îäíèõ è òåõ æå äàííûõ áîëüøå 20 ñåê.
	{
...
void CCar::SetOwnerHolderID(u16 id) // Âûñòàâèì ID ìàøèíû â ñåðâåðíîì îáúåêòå, ÷òîáû ïðèìåíèòü íîâûì êëèåíòàì ïðè ïîäêëþ÷åíèè
{
	if (OnServer())
...
void CCar::Teleport(Fvector outPos)
{
	...
	Fmatrix F = XFORM();
	Fvector pos = outPos;
	pos.y += 2.5; // TSMP: íà âñÿêèé ñëó÷àé ïîäíèìåì ïîâûøå òî÷êó ïîÿâëåíèÿ
	F.c = pos;
