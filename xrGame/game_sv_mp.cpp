...
void game_sv_mp::OnVoteStart(LPCSTR VoteCommand, ClientID const &sender)
{
	...
	m_server->ForEachClientDo([&sender, &pStartedPlayer](IClient* client)
	{
...
void game_sv_mp::UpdateVote()
{
	...
	m_server->ForEachClientDo([&NumParticipated, &NumAgreed,&NumToCount](IClient* client)
	{
		xrClientData* l_pC = dynamic_cast<xrClientData*>(client);
...
void game_sv_mp::DumpOnlineStatistic()
{
	...
	m_server->ForEachClientDo([this, &ini, &idx](IClient* client)
	{
...
void game_sv_mp::DumpRoundStatistics()
{
	...
	m_server->ForEachClientDo([this, &idx, &ini](IClient* client)
	{
			xrClientData* l_pC = static_cast<xrClientData*>(client);
			if (m_server->GetServerClient() == l_pC && g_dedicated_server)
				return;
