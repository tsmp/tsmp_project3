...
public:
	...
	CSE_Abstract *SpawnObject(const char* section, const Fvector &pos, const char* customData = nullptr, ALife::_OBJECT_ID parent = -1);
	...
	// WARNING: this does not send event to xrServer, only broadcast to clients
	void SvEventSend(NET_Packet &P, u32 dwFlags = DPNSEND_GUARANTEED);
	...
	virtual void net_Export_Update(NET_Packet &P, ClientID const &idTo); // just incremental update for specific client
