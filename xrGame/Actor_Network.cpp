...
BOOL CActor::net_Spawn(CSE_Abstract *DC)
{
	...
	if (E->m_holderID != ALife::_OBJECT_ID(-1) && !g_dedicated_server)
...
void CActor::net_Destroy()
{
	...
	if (m_holder_id != ALife::_OBJECT_ID(-1) && !g_dedicated_server)
...
void CActor::ChangeVisual(shared_str NewVisual)
{
	...
	if (cNameVisual().size() && cNameVisual() == NewVisual)
		return;
...
void CActor::CalculateInterpolationParams()
{
	...
	if (m_bInInterpolation)
	{
		...
		for (u32 k = 0; k < 3; k++)
		{
			...
			HP1[k] = (c * c * HCoeff[k][0] * 3 + c * HCoeff[k][1] * 2 + HCoeff[k][2]); // скорость из формулы в 3 раза превышает скорость при расчете коэффициентов !!!!
		}
