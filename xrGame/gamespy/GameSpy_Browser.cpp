...
void CGameSpy_Browser::ReadServerInfo(ServerInfo *pServerInfo, void *pServer)
{
	...
	pServerInfo->m_aInfos.push_back(
		GameInfo(*st.translate("mp_si_voting"),
				 (xrGS_SBServerGetBoolValue(pServer, m_pQR2->xrGS_RegisteredKey(G_VOTING_ENABLED_KEY), SBFalse) == SBTrue) ? *st.translate("mp_si_enabled") : *st.translate("mp_si_disabled")));
