...
void capped_cylinder_ray_collision_test()
{
	...
	for (int i = 0; i < 1000000; i++)
	{
		...
		//ray
		Fvector dir2, pos2;
		float R2 = Random.randF(0.1f, 2.f);
		dir2.random_dir();
		pos2.random_point(Fvector().set(2, 2, 2));
		RAYvsCYLINDER(c2, pos2, dir2, R2, TRUE);
	}
	...
	{
		Fcylinder c3;
		c3.m_center.random_point(Fvector().set(2, 2, 2));
		c3.m_direction.random_dir();
		c3.m_height = Random.randF(0.2f, 2.f);
		c3.m_radius = Random.randF(0.1f, 2.f);
		//ray
		Fvector dir3, pos3; //float R=Random.randF(0.1f,2.f);
		dir3.random_dir();
		pos3.random_point(Fvector().set(2, 2, 2));
		c3.intersect(pos3, dir3, ir);
	}
