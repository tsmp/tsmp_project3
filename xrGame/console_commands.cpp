...
extern BOOL g_bUseCursorAcceleration;
...
public:
	CCC_Net_CL_InputUpdateRate(LPCSTR N, int *V, int _min = 0, int _max = 999) : CCC_Integer(N, V, _min, _max),
																				 value_blin(V){};
...
public:
	CCC_Net_SV_GuaranteedPacketMode(LPCSTR N, int *V, int _min = 0, int _max = 2) : CCC_Integer(N, V, _min, _max),
																					value_blin(V){};
...
void CCC_RegisterCommands()
{
	...
	CMD4(CCC_Float, "fov", &g_fov, 55.0f, 90.0f);
	...
	CMD4(CCC_Integer, "g_car_camera_view", &g_car_camera_view, 0, 2);
	CMD4(CCC_Integer, "car_arms_on_steering_wheel", &show_arms_on_steering_wheel, 0, 1);
	CMD4(CCC_Integer, "net_log_events_gen", &log_events_gen, 0, 1);
