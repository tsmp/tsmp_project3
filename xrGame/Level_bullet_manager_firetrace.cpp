				...
				if (actor && IsGameTypeSingle() /**/ || stalker /**/)
				{
					...
					if (Fsphere::rpNone != S.intersect_full(bullet->pos, bullet->dir, dist))
					{
						...
						if (actor)
						{
							...
							if (CAI_Stalker* stalk = smart_cast<CAI_Stalker*>(initiator))
								hpf = stalk->SpecificCharacter().hit_probability_factor();
...
void CBulletManager::DynamicObjectHit(CBulletManager::_event &E)
{
	...
	if (E.bullet.flags.allow_sendhit && !E.Repeated)
	{
		...
		if (GameID() != GAME_SINGLE && E.bullet.flags.allow_sendhit && E.R.O->CLS_ID == CLSID_OBJECT_ACTOR && Game().m_WeaponUsageStatistic->CollectData())
		{
												  ...
												  Fvector &hit_normal)
{
	...
	if (((f + shoot_factor) < ricoshet_factor) && bullet->flags.allow_ricochet)
	{
		...
		float energyLost = 1.f - bullet->speed / old_speed;
		//импульс переданный объекту равен прямопропорционален потерянной энергии
		impulse = bullet->hit_impulse * speed_factor * energyLost;
