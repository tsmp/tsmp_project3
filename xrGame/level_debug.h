...
public:
	...
	public:
		IC void add_item(T data)
		{
			...
			sort(m_data.begin(), m_data.end(), sort_id_pred());
		}

		IC void remove_item(LPCSTR text)
		{
			ITEM_STORAGE_VEC_IT it = remove_if(m_data.begin(), m_data.end(), remove_text_pred(text));
		...
		{
			ITEM_STORAGE_VEC_IT it = remove_if(m_data.begin(), m_data.end(), remove_id_pred(id));
			m_data.erase(it, m_data.end());
