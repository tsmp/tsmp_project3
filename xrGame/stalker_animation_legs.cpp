...
MotionID CStalkerAnimationManager::legs_move_animation()
{
	...
	if (eMentalStateDanger != movement.mental_state())
	{
		...
		return (
			m_data_storage->m_part_animations.A[body_state()].m_movement.A[movement.movement_type()].A[eMovementDirectionForward].A[1]);
	}
	...
	return (
		m_data_storage->m_part_animations.A[body_state()].m_movement.A[movement.movement_type()].A[speed_direction].A[0]);
}
