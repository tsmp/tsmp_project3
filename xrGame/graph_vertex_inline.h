...
{
	EDGES::const_iterator I = find(edges().begin(), edges().end(), vertex_id);
	if (m_edges.end() == I)
		return (0);
	return (&*I);
}
...
{
	EDGES::iterator I = find(m_edges.begin(), m_edges.end(), vertex_id);
	if (m_edges.end() == I)
		return (0);
	return (&*I);
}
...
{
	EDGES::iterator I = find(m_edges.begin(), m_edges.end(), vertex->vertex_id());
...
{
	VERTICES::const_iterator I = find(m_vertices.begin(), m_vertices.end(), vertex);
...
{
	VERTICES::iterator I = find(m_vertices.begin(), m_vertices.end(), vertex);
	VERIFY(I != m_vertices.end());
	m_vertices.erase(I);
}
