...
{
	if (m_object->g_Alive() && (pHDS->hit_type == ALife::eHitTypeFireWound) && (Device.CurrentFrameNumber != m_last_hit_frame))
	{
	...
	m_last_hit_frame = Device.CurrentFrameNumber;
}
	...
	for (u32 i = 0; i < TRACE_ATTEMPT_COUNT; i++)
	{
		...
		if (Level().ObjectSpace.RayPick(position, dir, TRACE_DISTANCE, collide::rqtStatic, l_rq, NULL))
		{
			if (l_rq.range < TRACE_DISTANCE)
			{
				...
				{
					CLONE_MTL_SOUND(m_strange_sound, mtl_pair);
					Fvector pos;
