...
void CStateMonsterEatAbstract::setup_substates()
{
	...
	if (current_substate == eStateEat_CorpseApproachRun)
	{
		...
		const CEntityAlive *corpseEntity = object->CorpseMan.get_corpse();

		if (!corpseEntity->m_pPhysicsShell || !corpseEntity->m_pPhysicsShell->isActive())
			nearest_bone_pos = corpseEntity->Position();
		else
			nearest_bone_pos = object->character_physics_support()->movement()->PHCaptureGetNearestElemPos(corpseEntity);
