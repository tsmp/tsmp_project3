...
#if RENDER == R_R1
	if (RI.o.vis_intersect && (pVisual->vis.accept_frame != Device.CurrentFrameNumber))
		return;
	pVisual->vis.accept_frame = Device.CurrentFrameNumber;
#endif
...
void CRender::add_leafs_Dynamic(dxRender_Visual *pVisual)
{
	...
	case MT_PARTICLE_GROUP:
	{
		...
		{
			PS::CParticleGroup::SItem &Itm = *i_it;
			if (Itm._effect)
				add_leafs_Dynamic(Itm._effect);
			for (xr_vector<dxRender_Visual *>::iterator pit = Itm._children_related.begin(); pit != Itm._children_related.end(); pit++)
				add_leafs_Dynamic(*pit);
			for (xr_vector<dxRender_Visual *>::iterator pit = Itm._children_free.begin(); pit != Itm._children_free.end(); pit++)
				add_leafs_Dynamic(*pit);
		}
	}
		return;
...
BOOL CRender::add_Dynamic(dxRender_Visual *pVisual, u32 planes)
{
	...
	case MT_PARTICLE_GROUP:
	{
		...
		for (PS::CParticleGroup::SItemVecIt i_it = pG->items.begin(); i_it != pG->items.end(); i_it++)
		{
			...
			{
				if (Itm._effect)
					add_Dynamic(Itm._effect, planes);
				for (xr_vector<dxRender_Visual *>::iterator pit = Itm._children_related.begin(); pit != Itm._children_related.end(); pit++)
	...
	case MT_SKELETON_RIGID:
	{
		...
		if (pV->m_lod)
		{
			Fvector T_pos;
			float D;
			val_pTransform->transform_tiny(T_pos, pV->vis.sphere.P);
			float ssa = CalcSSA(D, T_pos, pV->vis.sphere.R / 2.f); // assume dynamics never consume full sphere
			if (ssa < r_ssaLOD_A)
				_use_lod = TRUE;
		}
		if (_use_lod)
		...
		/*
			Itm = pV->children.begin		();
			E = pV->children.end		();
			if (fcvPartial==VIS) {
				for (; Itm!=E; Itm++)	add_Dynamic			(*Itm,planes);
			} else {
				for (; Itm!=E; Itm++)	add_leafs_Dynamic	(*Itm);
			}
			*/
	}
	break;
