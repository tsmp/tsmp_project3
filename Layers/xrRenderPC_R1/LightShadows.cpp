...
void CLightShadows::set_object(IRenderable *O)
{
	...
	{
		if (!O->renderable_ShadowGenerate() || RImplementation.val_bHUD || ((CROS_impl *)O->renderable_ROS())->shadow_gen_frame == Device.CurrentFrameNumber)
		{
			current = 0;
			return;
		}
		...
		if (current)
		{
			((CROS_impl *)O->renderable_ROS())->shadow_gen_frame = Device.CurrentFrameNumber;
...
void CLightShadows::calculate()
{
	...
	for (u32 o_it = 0; o_it < casters.size(); o_it++)
	{
		...
		for (u32 l_it = 0; (l_it < lights.size()) && (slot_id < slot_max); l_it++)
		{
			...
			D3DVIEWPORT9 VP = { (DWORD)s_x * S_size, (DWORD)s_y * S_size, (DWORD)S_size, (DWORD)S_size, 0, 1};
