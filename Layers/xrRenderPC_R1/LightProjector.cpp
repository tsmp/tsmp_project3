...
void CLightProjector::set_object(IRenderable *O)
{
	...
	{
		if (!O->renderable_ShadowReceive() || RImplementation.val_bInvisible || ((CROS_impl *)O->renderable_ROS())->shadow_recv_frame == Device.CurrentFrameNumber)
		{
			current = 0;
			return;
		}
		...
		if (current)
		{
			...
			LT->shadow_recv_frame = Device.CurrentFrameNumber;
...
void CLightProjector::calculate()
{
	...
	for (u32 r_it = 0; r_it < receivers.size(); r_it++)
	{
		...
		if (bValid)
		{
			...
			cache[slot].dwFrame = Device.CurrentFrameNumber;
		}
		else
		{
	...
	for (u32 c_it = 0; c_it < cache.size(); c_it++)
	{
		if (taskid.empty())
			break;
		if (Device.CurrentFrameNumber == cache[c_it].dwFrame)
			continue;
		...
		if ((v.x * v.x + v.y * v.y + v.z * v.z) <= flt_zero)
		{
			...
			LT->shadow_recv_frame = Device.CurrentFrameNumber - 1;
		...
		D3DVIEWPORT9 VP = {(DWORD)s_x * P_o_size, (DWORD)s_y * P_o_size, (DWORD)P_o_size, (DWORD)P_o_size, 0, 1};
