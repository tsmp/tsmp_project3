...
namespace text_editor
{
	...
	void line_edit_control::on_key_release(int dik)
	{
		...
		{
		case DIK_LSHIFT:
			set_key_state(ks_LShift, false);
			break;
		case DIK_RSHIFT:
			set_key_state(ks_RShift, false);
			break;
		case DIK_LCONTROL:
			set_key_state(ks_LCtrl, false);
			break;
		case DIK_RCONTROL:
			set_key_state(ks_RCtrl, false);
			break;
		case DIK_LALT:
			set_key_state(ks_LAlt, false);
			break;
		case DIK_RALT:
			set_key_state(ks_RAlt, false);
			break;
		}
		update_bufs();
	}
	...
	void line_edit_control::on_frame()
	{
		...
		if (m_last_changed_frame + 1 < Device.CurrentFrameNumber)		
			m_need_update = false;
	}
	...
	void line_edit_control::update_bufs()
	{
		...
		m_need_update = true;
		m_last_changed_frame = Device.CurrentFrameNumber;
	}
