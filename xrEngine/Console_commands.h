...
public:
	CCC_Mask(LPCSTR N, Flags32 *V, u32 M) : IConsole_Command(N),
											value(V),
											mask(M){};
...
public:
	CCC_ToggleMask(LPCSTR N, Flags32 *V, u32 M) : IConsole_Command(N),
												  value(V),
												  mask(M)
	{
...
public:
	CCC_Token(LPCSTR N, u32 *V, xr_token *T) : IConsole_Command(N),
											   value(V),
											   tokens(T){};
...
public:
	CCC_Float(LPCSTR N, float *V, float _min = 0, float _max = 1) : IConsole_Command(N),
																	value(V),
																	min(_min),
																	max(_max){};
...
public:
	CCC_Vector3(LPCSTR N, Fvector *V, const Fvector _min, const Fvector _max) : IConsole_Command(N),
																				value(V)
	{
		min.set(_min);
		max.set(_max);
...
public:
	CCC_String(LPCSTR N, LPSTR V, int _size = 2) : IConsole_Command(N),
												   value(V),
												   size(_size)
	{
