...
void CBackend::dbg_Draw(D3DPRIMITIVETYPE _T, FVF::L *pVerts, int vcnt, u16 *pIdx, int pcnt)
{
	OnFrameEnd();
	CHK_DX(HW.pDevice->SetFVF(FVF::F_L));
	CHK_DX(HW.pDevice->DrawIndexedPrimitiveUP(_T, 0, vcnt, pcnt,
											  pIdx, D3DFMT_INDEX16,
											  pVerts, sizeof(FVF::L)));
}
void CBackend::dbg_Draw(D3DPRIMITIVETYPE _T, FVF::L *pVerts, int pcnt)
{
	OnFrameEnd();
	CHK_DX(HW.pDevice->SetFVF(FVF::F_L));
	CHK_DX(HW.pDevice->DrawPrimitiveUP(_T, pcnt, pVerts, sizeof(FVF::L)));
}

#define RGBA_GETALPHA(rgb) ((rgb) >> 24)
void CBackend::dbg_DrawOBB(Fmatrix &_T, Fvector &half_dim, u32 _C)
{
	Fmatrix mL2W_Transform, mScaleTransform;

	mScaleTransform.scale(half_dim);
	mL2W_Transform.mul_43(_T, mScaleTransform);

	FVF::L aabb[8];
	aabb[0].set(-1, -1, -1, _C); // 0
	aabb[1].set(-1, +1, -1, _C); // 1
	aabb[2].set(+1, +1, -1, _C); // 2
	aabb[3].set(+1, -1, -1, _C); // 3
	aabb[4].set(-1, -1, +1, _C); // 4
	aabb[5].set(-1, +1, +1, _C); // 5
	aabb[6].set(+1, +1, +1, _C); // 6
	aabb[7].set(+1, -1, +1, _C); // 7
...
void CBackend::dbg_DrawTRI(Fmatrix &_T, Fvector &p1, Fvector &p2, Fvector &p3, u32 _C)
{
	FVF::L tri[3];
	tri[0].p = p1;
	tri[0].color = _C;
...
void CBackend::dbg_DrawLINE(Fmatrix &_T, Fvector &p1, Fvector &p2, u32 _C)
{
	FVF::L line[2];
	line[0].p = p1;
	line[0].color = _C;
...
void CBackend::dbg_DrawEllipse(Fmatrix &_T, u32 _C)
{
	float gVertices[] =
		{
	...
	{
		int k = i * 3;
		verts[i].set(gVertices[k], gVertices[k + 1], gVertices[k + 2], _C);
	}
