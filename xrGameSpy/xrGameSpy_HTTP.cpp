...
XRGAMESPY_API GHTTPRequest xrGS_ghttpSave(
...
{
	return ghttpSave(URL, filename, blocking, completedCallback, param);
}

XRGAMESPY_API GHTTPRequest xrGS_ghttpSaveEx(
	const gsi_char *URL,					  // The URL for the file ("http://host.domain[:port]/path/filename").
	const gsi_char *filename,				  // The path and name to store the file as locally.
...
{
	return ghttpSaveEx(URL, filename, headers, post, throttle, blocking, progressCallback, completedCallback, param);
}
