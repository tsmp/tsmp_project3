...
namespace eastl
{
	...
	vector<T, Allocator>::operator=(std::initializer_list<value_type> ilist)
	{
		...
		DoAssignFromIterator<InputIterator, false>(ilist.begin(), ilist.end(), ITC()); // initializer_list has const elements and so we can't move from them.
		return *this;
	}
	...
	{
		typedef typename eastl::iterator_traits<InputIterator>:: iterator_category ITC;
		DoInitFromIterator(first, last, ITC());
	}
	...
	{
		typedef typename eastl::iterator_traits<InputIterator>::iterator_category ITC;
		DoAssignFromIterator<InputIterator, bMove>(first, last, ITC());
	}
	...
	{
		typedef typename eastl::iterator_traits<InputIterator>::iterator_category ITC;
		DoInsertFromIterator(position, first, last, ITC());
	}
